...
void Data_Loader::Display_Gray_ASCII(int w, int h, int **pixels){
    ...
    for (int y = 0; y < h; ++y) {
        for (int x = 0; x < w; ++x) {
            ...
            int gray = pixels[y][x];
            std::cout << "\033[48;2;" << gray << ";" << gray << ";" << gray << "m"
                      << "\033[38;2;" << gray << ";" << gray << ";" << gray << "m"<< shades[index] << shades[index] << "\033[0m";
        }
        std::cout << std::endl;
    }
}
...
void Data_Loader::Display_RGB_ASCII(int w, int h, int ***pixels){
    ...
    for (int y = 0; y < h; ++y) {
        for (int x = 0; x < w; ++x) {
            ...
            int index = intensity * strlen(shades) / 255; // Scale intensity to match shades
            std::cout <<"\033[48;2;"<<pixels[y][x][0]<<";"<<pixels[y][x][1]<<";"<<pixels[y][x][2]<<"m"<<"\033[38;2;"<<pixels[y][x][0]<<";"<<pixels[y][x][1]<<";"<<pixels[y][x][2]<<"m"<<shades[index] <<""<< shades[index]<<"\033[0m";
        }
        std::cout << std::endl;
    }
}
