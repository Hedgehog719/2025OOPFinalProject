...
void RSAEncrypt(std::string &s, long long e_key, long long n_mod) {
    ...
    if (n_mod <= 255 && n_mod > 1) { // 僅在 n 較小時提示字元問題
    } else if (n_mod <=1) {
    ...
    for (char &c : s) {
        long long m = static_cast<unsigned char>(c); // 原始 ASCII 值
...
*/
    ...
    while(true){
        ...
        cout << "Please enter the number of image file: ";
        cin>>imgchoose;
        if(imgchoose==0){
            cout<<"Exit!"<<endl;
            break;
        ...
        if(choice==1){
            ...
            /*img3->DumpImage("sametest.jpg");
            Image *img4 = new GrayImage();
            img4->LoadImage("sametest.jpg");
            //img4->Display_X_Server();
            if(img3->IsSameAs(*img4)){
                cout<<"same"<<endl;
            }else{
                cout<<"not same"<<endl;
            }*/
            ...
            img3 = new RGBImage();
            img3->LoadImage(filename);
            /*img3->DumpImage("sametest.png");
            Image *img4 = new RGBImage();
            img4->LoadImage("sametest.png");
            //img4->Display_X_Server();
            if(img3->IsSameAs(*img4)){
                cout<<"same"<<endl;
            }else{
        ...
        uint8_t options = 0b00000000; // Initialize options to 0
        char yn;
        ...
        if(yn == 'y' || yn == 'Y'){
            ...
            cin>>yn;
            if(yn == 'y' || yn == 'Y'){
                options |= CASE_ONE; // Set the first bit
            }
            cout<<"2.Inversion?(y/n) ";
            cin>>yn;
            if(yn == 'y' || yn == 'Y'){
                options |= CASE_TWO; // Set the second bit
            }
            cout<<"3.Denoising?(y/n) ";
            cin>>yn;
            if(yn == 'y' || yn == 'Y'){
                options |= CASE_THREE; // Set the third bit
            }
            cout<<"4.Thresholding?(y/n) ";
            cin>>yn;
            if(yn == 'y' || yn == 'Y'){
                options |= CASE_FOUR; // Set the fourth bit
            }
            cout<<"5.Sharpening?(y/n) ";
            cin>>yn;
            if(yn == 'y' || yn == 'Y'){
                options |= CASE_FIVE; // Set the fifth bit
            }
            cout<<"6.Mosaic?(y/n) ";
            cin>>yn;
            if(yn == 'y' || yn == 'Y'){
                options |= CASE_SIX; // Set the sixth bit
            }
            cout<<"7.Flip Horizontal?(y/n) ";
            cin>>yn;
            if(yn == 'y' || yn == 'Y'){
                options |= CASE_SEVEN; // Set the sixth bit
            }
            cout<<"8.Fish Eye?(y/n) ";
            cin>>yn;
            if(yn == 'y' || yn == 'Y'){
        ...
        if(yn == 'y' || yn == 'Y'){
            ...
            char len = static_cast<char>(message.length());  // 將長度轉為字元
            message.insert(0, 1, len);
            
            img3->EncryptMessage(message);  // 呼叫剛剛你實作的加密函數
            ...
            char len = static_cast<char>(message.length());  // 將長度轉為字元
            message.insert(0, 1, len);
            img3->EncryptMessage(message);  // 將訊息加密到原圖 img3
        ...
        /*
            ...
            */
            Image *img4 =img3->Clone();//nullptr
            //img4->LoadImage("saveimg/"+Decodename+".png");
            string demessage;
            ...
            // 若需另存圖片（不一定需要這行）
            // img4->DumpImage("decoded_copy.jpg");
            delete img4;
